...
def get_pie_chart(entered_site):
    if entered_site=='ALL':
        ...
        fig = px.pie(success_counts, values='success_count', names='Launch Site', title='Total Successful Launches by Site')
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        outcome_counts = filtered_df['class'].value_counts().reset_index()
        outcome_counts.columns = ['class', 'count']
        outcome_counts['class'] = outcome_counts['class'].map({0: 'Failure', 1: 'Success'})
        fig = px.pie(outcome_counts, values='count', names='class', title=f'Success vs Failure launches at site: {entered_site}')
    return fig
